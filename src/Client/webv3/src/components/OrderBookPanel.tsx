...
export function OrderBookPanel() {
  ...
  const orderBooks = useGlobalStore((state) => state.order_book_per_security_color);
      ...
      <div className='flex flex-row gap-2 flex-grow flex-shrink overflow-hidden'>
        <div className='w-1/2 border-1 flex flex-col h-full'>
          ...
          <div className='overflow-y-scroll flex-1'>
            {orderBooks[selectedSecurity]?.bid?.map((order) => (
              <div
                key={order.order_id}
                className={clsx(
                  "grid grid-cols-3 text-sm border-b",
                  order.color === "green" && "bg-green-300",
                  order.color === "red" && "bg-red-300"
                )}
              >
                <div className='px-2 py-1 truncate'>
                  <span>{userIdMap[order.user_id] ?? order.user_id}</span>
                </div>
                <div className='px-2 py-1 text-right'>
                  <span>{order.volume}</span>
                  {order.change !== undefined && <span> (-{order.change})</span>}
                </div>
        ...
        <div className='w-1/2 border-1 flex flex-col h-full'>
          ...
          <div className='overflow-y-scroll flex-1'>
            {orderBooks[selectedSecurity]?.ask?.map((order) => (
              <div
                key={order.order_id}
                className={clsx(
                  "grid grid-cols-3 text-sm border-b",
